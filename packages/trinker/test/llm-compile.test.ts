...
describe("llm compile: the proposal reaches deterministic validation", () => {
  it("accepts a valid proposal and reports what was added", async () => {
    ...
    const result = await llmCompileProject(root, { tokenBudget: 50_000, provider: fakeProvider(() => goodProposal(routeId)) });
  ...
  it("stamps llm-assisted provenance on the plan and its invariants", async () => {
    const root = await project();
    const routeId = await routeIdOf(root, "/api/orders/:id");
    const result = await llmCompileProject(root, { tokenBudget: 50_000, apply: true, provider: fakeProvider(() => goodProposal(routeId)) });
  ...
  it("rejects a proposal naming a route that does not exist", async () => {
    ...
    const result = await llmCompileProject(root, { tokenBudget: 50_000, provider: fakeProvider(() => proposal) });
  ...
  it("refuses a proposal that is prose rather than structured plan content", async () => {
    const root = await project();
    await expect(llmCompileProject(root, { tokenBudget: 50_000, provider: fakeProvider(() => "I found an IDOR in /api/orders/:id") }))
  ...
  it("refuses a proposal carrying a credential, rather than stripping it and continuing", async () => {
    ...
    await expect(llmCompileProject(root, { tokenBudget: 50_000, provider: fakeProvider(() => proposal) }))
  ...
  it("does not let a proposal redefine an already-authored check", async () => {
    const root = await project();
    const routeId = await routeIdOf(root, "/api/orders/:id");
    await llmCompileProject(root, { tokenBudget: 50_000, apply: true, provider: fakeProvider(() => goodProposal(routeId)) });

    const second = await llmCompileProject(root, { tokenBudget: 50_000, provider: fakeProvider(() => goodProposal(routeId)) });
...
describe("llm compile: budget and telemetry", () => {
  it("refuses a compilation whose estimate exceeds the budget, without calling the provider", async () => {
    ...
    await expect(llmCompileProject(root, { tokenBudget: 1000, provider })).rejects.toThrow(/token budget/);
  ...
  it("reports an overrun even when the provider under-estimated", async () => {
    ...
    await expect(llmCompileProject(root, { tokenBudget: 5000, provider })).rejects.toThrow(/exceeded its token budget/);
  });
...
describe("llm compile: the context boundary", () => {
  it("gives the provider the surface and the constraints, and no runtime data", async () => {
    ...
    await llmCompileProject(root, { tokenBudget: 50_000, provider });
